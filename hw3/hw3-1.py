...
class LinearModel:
    def __init__(
        self, learning_rate=0.5, iterations=1000, model_type="linear"
    ) -> None:
...
class DecisionTree:
    def __init__(self, max_depth: int = 10, model_type: str = "classifier"):
...
class RandomForest:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        for tree in self.trees:
            # TODO: 2%
            bootstrap_indices = np.random.choice(np.arange(total_data_num), size=total_data_num*1//2, replace=False)
